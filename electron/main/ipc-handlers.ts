    ...
    ipcMain.handle("storeAdd", async (_, storeName, key, value) => {
        ...
        if (storeName === "customerData" && key === "customerData") {
            try {
                ...
                const serviceContainer = await getServices();
                const createdCustomer = await serviceContainer.customerService.create(value);
        ...
        if (storeName === "invoiceData" && key === "invoiceData") {
            try {
                ...
                const serviceContainer = await getServices();
                const createdInvoice = await serviceContainer.invoiceService.create(value, value.lineItems || []);
                console.log(`Created invoice: ${createdInvoice._id}`);
        ...
        if (storeName === "projectData" && key === "projectData") {
            try {
                ...
                const serviceContainer = await getServices();
                const createdProject = await serviceContainer.projectService.create(value);
                console.log(`Created project: ${createdProject._id}`);
